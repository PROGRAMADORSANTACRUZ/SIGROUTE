...
// GET /api/ordenes/preasignacion-hoy?fecha= — solo informativo: qué rutas de
// Preasignación (Planeación) ya tienen esa placa hoy, para avisar en Asignación
// de Órdenes sin tocar la lógica de Drivin ni el flujo de asignación real.
router.get("/preasignacion-hoy", requireAuth, async (req, res, next) => {
  try {
    ...
    const clientes = clienteIds.length
      ...
      : [];
    const clientePorId = new Map(clientes.map((c) => [c.id, c.cliente || c.nombreDireccion || "(sin nombre)"]));

    const porPlaca: Record<string, { numeroRuta: number; destinos: string[] }> = {};
    for (const r of rutas) {
      const placa = r.vehiculo?.placa?.toUpperCase().trim();
      if (!placa) continue;
      porPlaca[placa] = {
        numeroRuta: r.numeroRuta,
        destinos: r.destinos.map((d) => (d.clienteId ? clientePorId.get(d.clienteId) : null) ?? d.destino?.nombre ?? "(sin nombre)"),
      };
    }
    res.json(porPlaca);
  } catch (err) {
...
router.post(
  "/import",
  requireAuth,
  requirePermiso("distrilog.ordenes.ver"),
  upload.single("file"),
  async (req, res, next) => {
    try {
      if (!req.file) {
...
router.post("/reenviar", requireAuth, requirePermiso("distrilog.nivel_servicio.editar"), async (req, res, next) => {
  try {
...
router.post("/asignar", requireAuth, requirePermiso("distrilog.asignacion.editar"), async (req, res, next) => {
  try {
    const ids: unknown = req.body?.ids;
...
// Prueba cada formato de documento posible contra apiconsulta para una
// ventana de fechas dada; solo devuelve filas si alguno trajo resultados.
async function buscarFacturaEnSiesa(
  base: string,
  cia: string,
  candidatos: string[],
  fechaInicio: string,
  fechaFin: string
): Promise<{ documento: string; filas: TatInvoice[] }> {
...
// (`/tat-plantilla`) — misma lógica, sin duplicar el cruce.
async function resolverFacturaTat(params: {
  origen: string;
  numFac: string;
  fecFac: string;
  fecFin?: string;
  ...
  const fecFac = params.fecFac.trim().slice(0, 10);
  if (!numFac) throw new HttpError(400, "Falta el número de factura (NumFac)");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(fecFac)) throw new HttpError(400, "Fecha de factura inválida");
  let fecFin = String(params.fecFin ?? "").trim().slice(0, 10);
  if (fecFin && !/^\d{4}-\d{2}-\d{2}$/.test(fecFin)) throw new HttpError(400, "Fecha fin inválida");
  if (!fecFin) fecFin = fecFac;
  const rutaBody = String(params.ruta ?? "").trim().slice(0, 60) || null;
  ...
  let { documento, filas } = await buscarFacturaEnSiesa(base, cia, candidatos, fechaInicio, fechaFin);

  // La fecha capturada (ruta/despacho/plantilla) no siempre coincide con la
  // fecha real de facturación en Siesa (p.ej. se factura un día y se reparte
  // al siguiente, o el usuario digita la fecha de entrega). Si la búsqueda
  // exacta no encuentra nada, se reintenta con una ventana más amplia hacia
  // atrás (y un día hacia adelante) antes de descartar la factura.
  if (filas.length === 0) {
    const amplio = await buscarFacturaEnSiesa(base, cia, candidatos, sumarDias(fechaInicio, -6), sumarDias(fechaFin, 1));
    documento = amplio.documento;
    filas = amplio.filas;
  }
  if (filas.length === 0) {
...
router.post("/factura", requireAuth, requirePermiso("distrilog.ordenes.editar"), async (req, res, next) => {
  try {
    const resultado = await resolverFacturaTat({
      origen: String(req.body?.origen ?? "AGROPECUARIA"),
      numFac: String(req.body?.numFac ?? ""),
...
// auxiliar/ruta; se cruza automáticamente contra apiconsulta (Siesa) con la
// misma lógica de `/factura` y queda lista en Cargar Órdenes.
// ════════════════════════════════════════════════════════════════════════

// Columnas visibles de cada plantilla (para el Excel descargable y el mapeo
// al importar un archivo ya diligenciado). "extra" son columnas propias de
// cada plantilla que no tienen campo dedicado en Orden — se guardan tal cual
// en OrdenTatPlantilla.extra para trazabilidad/consulta.
const PLANTILLA_COLUMNAS: Record<string, { header: string; campo: string }[]> = {
  INVERSIONES: [
    { header: "Fecha Despacho", campo: "fecha" },
    { header: "Factura", campo: "numFac" },
    { header: "Cliente", campo: "extra.cliente" },
    { header: "Barrio", campo: "extra.barrio" },
    { header: "$ Valor", campo: "extra.valor" },
    { header: "Placas", campo: "placa" },
    { header: "Conductor", campo: "conductor" },
    { header: "Auxiliar", campo: "auxiliar" },
    { header: "Planilla", campo: "ruta" },
    { header: "Estado", campo: "extra.estado" },
    { header: "Novedades", campo: "extra.novedades" },
    { header: "Responsabilidad", campo: "extra.responsabilidad" },
    { header: "Detalle", campo: "extra.detalle" },
    { header: "Mes", campo: "extra.mes" },
  ],
  AGROPECUARIA: [
    { header: "NO. FACTURA", campo: "numFac" },
    { header: "NIT", campo: "extra.nit" },
    { header: "CLIENTES", campo: "extra.clientes" },
    { header: "DINERO", campo: "extra.dinero" },
    { header: "KILO FAC.", campo: "extra.kilo" },
    { header: "CONDUCTOR", campo: "conductor" },
    { header: "AUXLIAR", campo: "auxiliar" },
    { header: "PLACA", campo: "placa" },
    { header: "FECHA", campo: "fecha" },
    { header: "RUTA", campo: "ruta" },
    { header: "AZ", campo: "extra.az" },
    { header: "NG", campo: "extra.ng" },
    { header: "RJ", campo: "extra.rj" },
    { header: "GR", campo: "extra.gr" },
    { header: "VD", campo: "extra.vd" },
    { header: "CAJAS", campo: "extra.caja" },
    { header: "peso salida", campo: "extra.peso_salida" },
  ],
};

// Normaliza un encabezado para compararlo (sin acentos/espacios extra/mayúsculas),
// así el importador de Excel no se rompe por variaciones menores de tipeo en el
// archivo real de la gente (p.ej. "Kilo Fac" vs "KILO FAC.").
...
// dd/mm/aaaa (o serial de Excel) -> yyyy-mm-dd, para llamar a apiconsulta.
function fechaPlantillaAIso(value: unknown): string {
...
router.get("/tat-plantilla/formato", requireAuth, requirePermiso("distrilog.ordenes.editar"), (req, res, next) => {
  try {
    ...
    if (!columnas) throw new HttpError(400, "Origen inválido (AGROPECUARIA o INVERSIONES)");
    const ws = XLSX.utils.aoa_to_sheet([columnas.map((c) => c.header)]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Plantilla");
    const buf = XLSX.write(wb, { type: "buffer", bookType: "xlsx" }) as Buffer;
    res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    res.setHeader("Content-Disposition", `attachment; filename="plantilla-tat-${origen.toLowerCase()}.xlsx"`);
    res.send(buf);
  } catch (err) {
    next(err);
  }
...
// Procesa un lote de filas ya parseadas: guarda el registro crudo, cruza cada
// una contra Siesa (reutilizando resolverFacturaTat) y reporta éxito/error
// por fila SIN abortar el resto del lote. Si cruza bien, también devuelve el
// resumen (cliente/nit/barrio/kg/valor) para que la grilla se autorellene.
async function procesarFilasPlantillaTat(origen: string, filas: FilaPlantillaTat[]) {
  const resultados: {
  ...
  for (const fila of filas) {
    const numFac = String(fila.numFac ?? "").trim();
    const fechaIso = fechaPlantillaAIso(fila.fecha);
    if (!numFac) continue; // fila vacía, se ignora
    let estado: "CRUZADO" | "ERROR" = "ERROR";
    let mensaje: string | undefined;
    let resumen: Awaited<ReturnType<typeof resolverFacturaTat>> | null = null;
    try {
      if (!fechaIso) throw new HttpError(400, "Fecha inválida");
      resumen = await resolverFacturaTat({
        origen,
        numFac,
        fecFac: fechaIso,
        ruta: fila.ruta ?? undefined,
        placaInicial: fila.placa ?? undefined,
      });
      estado = "CRUZADO";
    } catch (err) {
      mensaje = err instanceof HttpError ? err.message : (err as Error)?.message ?? "Error desconocido";
    }
    await prisma.ordenTatPlantilla.upsert({
      where: { origen_numeroOrden: { origen, numeroOrden: numFac } },
      update: {
        fecha: String(fila.fecha ?? ""), placa: fila.placa || null, conductor: fila.conductor || null,
        auxiliar: fila.auxiliar || null, ruta: fila.ruta || null,
        extra: fila.extra ? JSON.stringify(fila.extra) : null, estado, mensaje: mensaje ?? null,
      },
      create: {
        origen, numeroOrden: numFac, fecha: String(fila.fecha ?? ""), placa: fila.placa || null,
        conductor: fila.conductor || null, auxiliar: fila.auxiliar || null, ruta: fila.ruta || null,
        extra: fila.extra ? JSON.stringify(fila.extra) : null, estado, mensaje: mensaje ?? null,
      },
    }).catch(() => null); // el upsert es solo trazabilidad; nunca debe tumbar el lote
    resultados.push({
      numFac, estado, mensaje,
      ...(resumen ? { cliente: resumen.cliente, nit: resumen.nit, barrio: resumen.barrio, direccion: resumen.direccion, totalKg: resumen.totalKg, totalValor: resumen.totalValor } : {}),
    });
  }
  const cruzados = resultados.filter((r) => r.estado === "CRUZADO").length;
  return { total: resultados.length, cruzados, errores: resultados.length - cruzados, detalle: resultados };
}
...
router.post("/tat-plantilla", requireAuth, requirePermiso("distrilog.ordenes.editar"), async (req, res, next) => {
  try {
    ...
    const filas = Array.isArray(req.body?.filas) ? (req.body.filas as FilaPlantillaTat[]) : [];
    if (filas.length === 0) throw new HttpError(400, "No se recibieron filas para procesar");
    res.json(await procesarFilasPlantillaTat(origen, filas));
  } catch (err) {
    next(err);
  }
});
...
router.post("/tat-plantilla/importar", requireAuth, requirePermiso("distrilog.ordenes.editar"), upload.single("file"), async (req, res, next) => {
  try {
    ...
    const headerPorCampo = new Map(columnas.map((c) => [c.campo, normalizarHeader(c.header)] as const));
    const filas: FilaPlantillaTat[] = rows.map((row) => {
      // Mapa normalizado de los encabezados REALES del archivo subido, para que
      // pequeñas variaciones de tipeo/mayúsculas no rompan el cruce de columnas.
      const rowNormalizado = new Map(Object.keys(row).map((k) => [normalizarHeader(k), row[k]] as const));
      const get = (campo: string) => {
        const header = headerPorCampo.get(campo);
        return header ? rowNormalizado.get(header) : undefined;
      };
      const extra: Record<string, unknown> = {};
      for (const c of columnas) {
        if (c.campo.startsWith("extra.")) extra[c.campo.slice(6)] = get(c.campo);
      }
      return {
        fecha: String(get("fecha") ?? ""),
        numFac: String(get("numFac") ?? "").trim(),
        placa: String(get("placa") ?? "").trim() || null,
        conductor: String(get("conductor") ?? "").trim() || null,
        auxiliar: String(get("auxiliar") ?? "").trim() || null,
        ruta: String(get("ruta") ?? "").trim() || null,
        extra,
      };
    }).filter((f) => f.numFac);

    if (filas.length === 0) throw new HttpError(400, "El archivo no tiene filas con número de factura");
    res.json(await procesarFilasPlantillaTat(origen, filas));
  } catch (err) {
    next(err);
  }
...
router.get("/tat-plantilla/borrador", requireAuth, requirePermiso("distrilog.ordenes.editar"), async (req, res, next) => {
  try {
    const origen = String(req.query.origen ?? "AGROPECUARIA").toUpperCase();
    if (!PLANTILLA_COLUMNAS[origen]) throw new HttpError(400, "Origen inválido (AGROPECUARIA o INVERSIONES)");
    const jornada = jornadaKeyActual();
    const registros = await prisma.plantillaTatBorrador.findMany({ where: { origen, jornada }, orderBy: { posicion: "asc" } });
    res.json({ jornada, filas: registros.map((r) => JSON.parse(r.fila)) });
  } catch (err) {
    next(err);
  }
});
...
router.put("/tat-plantilla/borrador", requireAuth, requirePermiso("distrilog.ordenes.editar"), async (req, res, next) => {
  try {
    const origen = String(req.body?.origen ?? "AGROPECUARIA").toUpperCase();
    if (!PLANTILLA_COLUMNAS[origen]) throw new HttpError(400, "Origen inválido (AGROPECUARIA o INVERSIONES)");
    const filas: unknown[] = Array.isArray(req.body?.filas) ? req.body.filas : [];
    const jornada = jornadaKeyActual();
    await prisma.$transaction([
      prisma.plantillaTatBorrador.deleteMany({ where: { origen, jornada } }),
      ...(filas.length
        ? [prisma.plantillaTatBorrador.createMany({
            data: filas.map((f, i) => ({ origen, jornada, posicion: i, fila: JSON.stringify(f) })),
          })]
        : []),
    ]);
...
router.get("/tat-plantilla", requireAuth, requirePermiso("distrilog.ordenes.ver"), async (req, res, next) => {
  try {
    ...
    const registros = await prisma.ordenTatPlantilla.findMany({
      where, orderBy: { createdAt: "desc" }, take: 500,
    });
    res.json(registros.map((r) => ({ ...r, extra: r.extra ? JSON.parse(r.extra) : null })));
  } catch (err) {
    next(err);
  }
});
...
router.post("/eliminar", requireAuth, requirePermiso("distrilog.ordenes.editar"), async (req, res, next) => {
  try {
    const ids: unknown = req.body?.ids;
    if (!Array.isArray(ids) || ids.length === 0) {
...
router.delete("/", requireAuth, requirePermiso("distrilog.ordenes.editar"), async (req, res, next) => {
  try {
...
router.post("/sync-drivin-estado", requireAuth, requirePermiso("distrilog.nivel_servicio.editar"), async (_req, res, next) => {
  try {
