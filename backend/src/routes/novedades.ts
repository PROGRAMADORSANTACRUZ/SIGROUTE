...
const novedadSchema = z.object({
  ...
  estado: z.enum(["Pendiente", "En tramitaci�n", "Resuelto", "Cerrada"]).default("Pendiente"),
...
// POST /api/novedades
router.post("/", requireAuth, requirePermiso("distrilog.nivel_servicio.editar"), async (req, res, next) => {
  try {
...
// POST /api/novedades/enviar-nivel � env�a remisiones (con veh�culo asignado) al
// Nivel de Servicio sin generar DL. No duplica: si la remisi�n ya est� en el
// Nivel (por numeroOrden) se omite. El c�digo DL-xxxx se asigna despu�s, cuando
// la remisi�n pasa por Planificaci�n DL (ver planillas.ts POST).
const enviarNivelSchema = z.object({
  numerosOrden: z.array(z.string().trim().min(1)).min(1, "Selecciona al menos una remisi�n"),
});
router.post("/enviar-nivel", requireAuth, requirePermiso("distrilog.nivel_servicio.editar"), async (req, res, next) => {
  try {
...
router.patch("/:id", requireAuth, requirePermiso("distrilog.nivel_servicio.editar"), async (req, res, next) => {
  try {
...
router.delete("/:id", requireAuth, requirePermiso("distrilog.nivel_servicio.editar"), async (req, res, next) => {
  try {
