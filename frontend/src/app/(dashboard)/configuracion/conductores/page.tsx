...
export default function ConductoresPage() {
  const puedeEditar = usePermiso("distrilog.config.editar");
  ...
  return (
    <div className="flex h-full flex-col p-4 sm:p-6 lg:p-8">
      ...
      {showAuxiliares && (
        <AuxiliaresModal onClose={() => setShowAuxiliares(false)} puedeEditar={puedeEditar} />
      )}
    </div>
  );
}
...
function AuxiliaresModal({ onClose, puedeEditar }: { onClose: () => void; puedeEditar: boolean }) {
                ...
                <tr key={a.id} className="hover:bg-[#f9fbf7]">
                  ...
                  <td className="px-4 py-2.5 text-right">
                    {puedeEditar ? (
                      ...
                      <span className="text-xs text-[#c0c9bf]">—</span>
                    )}
                  </td>
                </tr>
              ))}
